...
FOLDER = os.getenv('folder', os.path.join(os.path.dirname(__file__), 'ResetScans'))
DEFAULT_SUFFIX = os.getenv('default_suffix', '[ResetScans]')
...
def extract_series_title(session, series_url):
    url = f"https://reset-scans.org{series_url}"
    response = session.get(url, timeout=30)
    response.raise_for_status()
    html = response.text
    ...
    if title_match:
        title = title_match.group(1).replace(' Manga – Read Online | RESET SCANS', '').strip()
        return title
    return None

# Extract chapter URLs for a given manga
def extract_chapter_urls(session, manga_url):
    full_url = f"https://reset-scans.org{manga_url}"
    response = session.get(full_url, timeout=30)
    response.raise_for_status()
    html = response.text.replace('\0', '')
    
    chapter_urls = re.findall(r'href="https://reset-scans\.org(/manga/[^/]+/chapter-[^/]+/)"', html)
...
def extract_image_urls(session, chapter_url):
    full_url = f"https://reset-scans.org{chapter_url}"
    response = session.get(full_url, timeout=30)
    response.raise_for_status()
    html = response.text.replace('\0', '')
...
def main():
    ...
    # Health check
    log("Performing health check on https://reset-scans.org...")
    try:
    ...
    for series_url in all_series_urls:
        ...
        for cbz_file in series_directory.glob("*.cbz"):
            # Extract chapter number from filename like "Title Ch.001 [ResetScans].cbz"
