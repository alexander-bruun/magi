...
# Configuration
# =============================================================================
CONFIG = get_scraper_config("comix", "Comix.to", "")
...
def extract_series_urls(session, page_num):
    ...
    Returns:
        tuple: (list of series data dicts, bool is_last_page, int last_page)
    """
    ...
    try:
        ...
        if not isinstance(data, dict):
            ...
            is_last_page = True
            return series_data, is_last_page, last_page
...
def extract_series_title(session, series_url):
    ...
    try:
        ...
        match = re.search(r'<h1[^>]*class="[^"]*title[^"]*"[^>]*>([^<]+)</h1>', html, re.IGNORECASE)
        if match:
            title = match.group(1).strip()
            return title
    except Exception as e:
        error(f"Failed to extract title from {series_url}: {e}")
...
def extract_chapter_urls(session, series_url):
    ...
    while page_num <= max_pages:
        ...
        for chapter in chapters:
            ...
            group_dict = chapter.get("scanlation_group")
            if group_dict:
                group_id = group_dict.get("scanlation_group_id", 0)
                if group_id == 0:
                    group = "Comix"
                else:
                    group = group_dict.get("name", "")
            else:
                if chapter.get("is_official") == 1:
                    group = "Official"
                else:
                    group = "Comix"
    ...
    all_chapters.sort(key=lambda x: x['num'], reverse=True)
...
def main():
    ...
    # Cache file for series data
    cache_file = Path(__file__).parent / 'comix_series_cache.json'
    ...
    if cache_file.exists():
        ...
        try:
            ...
            log(f"Loaded {len(all_series_data)} series from cache")
        except Exception as e:
            error(f"Failed to load cache: {e}")
            all_series_data = []
    
    if not all_series_data:
        log("Fetching series data from API...")
        # Fetch first page to get total pages
        series_data, is_last_page, last_page = extract_series_urls(session, 1)
        all_series_data.extend(series_data)
        log(f"Collected {len(series_data)} series from page 1 (total: {len(all_series_data)})")
        
        if not is_last_page:
            # Fetch remaining pages in parallel
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(extract_series_urls, session, p) for p in range(2, last_page + 1)]
                for future in concurrent.futures.as_completed(futures):
                    series_data, _, _ = future.result()
                    all_series_data.extend(series_data)
                    log(f"Collected {len(series_data)} series (total: {len(all_series_data)})")
        
        # Save to cache
        try:
            with open(cache_file, 'w') as f:
                json.dump(all_series_data, f)
            log(f"Saved {len(all_series_data)} series to cache")
        except Exception as e:
            error(f"Failed to save cache: {e}")
    
    log(f"Total series collected: {len(all_series_data)}")
    
    for series in all_series_data:
        series_url = series['series_url']
        
        # Extract chapters first to get the group
        chapters = extract_chapter_urls(session, series_url)
        if not chapters:
            continue
        
        # Get the primary group from the first chapter
        first_group = chapters[0].get('group', 'Comix') if chapters else 'Comix'
        ...
        # Get the primary group from the first chapter or extracted
        first_group = chapters[0].get('group', 'Comix') if chapters else 'Comix'
        
        # Clean the title (already done above)
        
        log(f"Processing series: {clean_title}")
