...
FOLDER = os.getenv('folder', os.path.join(os.path.dirname(__file__), 'ManhwaHub'))
DEFAULT_SUFFIX = os.getenv('default_suffix', '[ManhwaHub]')
ALLOWED_DOMAINS = ['manhwahub.net', 'cdn.manhwahub.net']
BASE_URL = 'https://manhwahub.net'
PRIORITY, HIGHER_PRIORITY_FOLDERS = get_priority_config('manhwahub')
...
def extract_series_urls(session, page_num):
    ...
    if page_num == 1:
        ...
        url = f"https://manhwahub.net/?page={page_num}"
...
def extract_chapter_urls(session, manga_url):
    ...
    chapter_urls = re.findall(r'href="https://manhwahub\.net(/webtoon/[^/]+/chapter-[^/"]+)"', html)
...
def extract_image_urls(session, chapter_url):
    ...
    for url in image_urls:
        url = url.strip()
        parsed = urlparse(url)
        if parsed.scheme in ('http', 'https') and parsed.netloc in ('cdn.manhwahub.net', 'manhwahub.net') and 'assets/images' not in parsed.path:
