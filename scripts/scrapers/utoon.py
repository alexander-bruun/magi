...
DEFAULT_SUFFIX = os.getenv('default_suffix', '[UToon]')
ALLOWED_DOMAINS = ['utoon.net']
BASE_URL = 'https://utoon.net'
PRIORITY, HIGHER_PRIORITY_FOLDERS = get_priority_config('utoon')
...
def extract_series_urls(session, page_num):
    ...
    if response.status_code >= 400:
        log(f"Page {page_num} returned status {response.status_code}, stopping pagination")
        return [], True
    ...
    if len(html) < 1000:
        log(f"Page {page_num} has very little content ({len(html)} chars), likely end of pagination")
        return [], True
    
    # Check if this is the last page by looking for pagination links
    is_last_page = 'next page-numbers' not in html and 'Next' not in html and 'page/' not in html
    ...
    series_urls = [url for url in series_urls if re.match(r'^/manga/[a-z0-9\-]+/$', url)]
...
def extract_image_urls(session, chapter_url):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            image_urls = re.findall(r'src="(https://utoon\.net/wp-content/uploads/WP-manga/data/manga_[a-z0-9]+/[a-z0-9]+/\d+\.(?:jpg|jpeg|png|webp))"', html)
...
async def process_chapter(ch_url, series_directory, clean_title, num, browser_factory):
    ...
    async with browser_class as browser:
        try:
            ...
            await page.goto(full_ch_url, wait_until='load', timeout=60000)  # 60 second timeout
            ...
            downloaded = await download_images_concurrent(unique_images, chapter_folder, page)
...
async def process_series(series_url, browser_factory, series_semaphore, download_semaphore):
    ...
    async with series_semaphore:
        ...
        async with browser_class as browser:
            try:
                ...
                chapter_patterns.extend(re.findall(rf'{re.escape(series_slug)}/chapter-[\d]+/', html))
                
                # Pattern 2: full URLs like https://utoon.net/manga/the-baby-raising-a-devil/chapter-21/
                full_url_patterns = re.findall(rf'https://utoon\.net/manga/{re.escape(series_slug)}/chapter-[\d]+/', html)
                chapter_patterns.extend([pattern.replace('https://utoon.net/manga/', '') for pattern in full_url_patterns])
                
                # Pattern 3: href attributes
                href_patterns = re.findall(rf'href="[^"]*/manga/{re.escape(series_slug)}/chapter-[\d]+/?"', html)
                for href in href_patterns:
                    # Extract the path part
                    match = re.search(rf'/manga/{re.escape(series_slug)}/chapter-[\d]+/?', href)
                    if match:
                ...
                log(f"Found {len(existing_chapters)} existing chapters: {sorted(existing_chapters) if existing_chapters else 'none'}")
                ...
                for ch_url, num in chapters_to_process:
                    task = process_chapter(ch_url, series_directory, clean_title, num, browser_factory)
                ...
                async def limited_task(task):
                    async with semaphore:
                        result = await task
                        await asyncio.sleep(2)  # Delay between chapters to avoid rate limiting
                        return result
...
def main():
    ...
    log("Starting UToon scraper")
    log(f"Output folder: {FOLDER}")
    log("Mode: Full Downloader")

    if HAS_CAMOUFOX:
        # Use async version with browser
...
async def main_async_optimized():
    ...
    log(f"Concurrency settings: {MAX_CONCURRENT_SERIES} series, {MAX_CONCURRENT_CHAPTERS} chapters, {MAX_CONCURRENT_DOWNLOADS} downloads")
    ...
    try:
        ...
        async with temp_browser_class as temp_browser:
            ...
            from camoufox_captcha import solve_captcha
            captcha_success = await solve_captcha(temp_page, captcha_type='cloudflare', challenge_type='interstitial')
            if not captcha_success:
                warn("Captcha solving may have failed, but continuing...")
        ...
        async with collect_browser_class as collect_browser:
            ...
            while True:
                try:
                    page_series, is_last_page = await extract_series_urls_async(collect_page, page_num)
                    if page_series:
        ...
        series_tasks = [process_series(series_url, create_browser, series_semaphore, download_semaphore) 
...
async def main_async():
    ...
    try:
        async with AsyncCamoufox(
            ...
            try:
                page_series, is_last_page = await extract_series_urls_async(page, page_num)
                if page_series:
                    all_series_urls.extend(page_series)
                    log(f"Found {len(page_series)} series on page {page_num}")
                else:
            ...
            # Process each series
            for idx, series_url in enumerate(all_series_urls[:1], 1):  # Test with first series only
                ...
                for ch_url in chapter_urls:
                    ...
                    image_urls = re.findall(r'src="\s*(https://utoon\.net/wp-content/uploads/WP-manga/data/manga_[a-z0-9]+/[a-z0-9]+/\d+\.(?:jpg|jpeg|png|webp))"', ch_html)
                    unique_images = list(dict.fromkeys(image_urls))
                    ...
                    for i, url in enumerate(unique_images):
                        ...
                        try:
                            ...
                            with open(file, 'wb') as f:
                                f.write(bytes(response) if isinstance(response, (list, bytes)) else response)
                            downloaded += 1
