...
FOLDER = os.getenv('folder', os.path.join(os.path.dirname(__file__), 'SpiderScans'))
DEFAULT_SUFFIX = os.getenv('default_suffix', '[SpiderScans]')
...
def extract_series_urls(session, page_num):
    if page_num == 1:
        ...
        url = f"https://spiderscans.xyz/new/page/{page_num}/"
...
def extract_series_title(session, series_url):
    url = f"https://spiderscans.xyz{series_url}"
    response = session.get(url, timeout=30)
    response.raise_for_status()
    html = response.text
...
def extract_chapter_urls(session, manga_url):
    full_url = f"https://spiderscans.xyz{manga_url}"
    ...
    ajax_url = f"https://spiderscans.xyz{manga_url}ajax/chapters/?t=1"
    try:
        ...
        chapter_urls = re.findall(r'href="https://spiderscans\.xyz(/manga/' + re.escape(manga_slug) + r'/chapter-[^/]+/)"', ajax_html)
    ...
    chapter_urls = re.findall(r'href="https://spiderscans\.xyz(' + re.escape(manga_url.rstrip('/')) + r'/chapter-[^/]+/)"', html)
    # Remove duplicates and sort by chapter number
    unique_urls = sorted(set(chapter_urls), key=lambda x: int(re.search(r'chapter-(\d+)', x).group(1)) if re.search(r'chapter-(\d+)', x) else 0)
    return unique_urls
...
def extract_image_urls(session, chapter_url):
    full_url = f"https://spiderscans.xyz{chapter_url}"
    response = session.get(full_url, timeout=30)
    response.raise_for_status()
...
def main():
    ...
    # Health check
    log("Performing health check on https://spiderscans.xyz...")
    try:
    ...
    for series_url in all_series_urls:
        ...
        for cbz_file in series_directory.glob("*.cbz"):
            # Extract chapter number from filename like "Title Ch.001 [SpiderScans].cbz"
