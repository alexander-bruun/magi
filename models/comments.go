...
func CreateComment(comment Comment) error {
	query := `
	INSERT INTO comments (user_username, target_type, target_slug, content, created_at, updated_at)
	VALUES (?, ?, ?, ?, ?, ?)
	`
	...
	_, err := db.Exec(query, comment.UserUsername, comment.TargetType, comment.TargetSlug, comment.Content, timestamps.CreatedAt.Unix(), timestamps.UpdatedAt.Unix())
	return err
}
...
func GetCommentsByTarget(targetType, targetSlug string) ([]Comment, error) {
	query := `
	SELECT id, user_username, target_type, target_slug, content, created_at, updated_at
	FROM comments
	...
	for rows.Next() {
		...
		err := rows.Scan(&comment.ID, &comment.UserUsername, &comment.TargetType, &comment.TargetSlug, &comment.Content, &createdAt, &updatedAt)
		if err != nil {
			return nil, err
		}
