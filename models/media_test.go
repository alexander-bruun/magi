...
func TestGetMediaBySlugAndLibraryFiltered(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT allow_registration, max_users, content_rating_limit,.*FROM app_config WHERE id = 1`).
		WillReturnRows(sqlmock.NewRows([]string{
			...
			1, 100, 0, "mangadex", "", "", "secret-key", 1, "pk_test", "sk_test", "whsec_test", 1, 100, 60, 1, 5, 10, 60, 70, 85, 100, 90, 70, 85, 5, 3600, 3, 0, 0, "We are currently performing maintenance. Please check back later.", 48,
		))
...
func TestGetTopMedias(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT allow_registration, max_users, content_rating_limit,.*FROM app_config WHERE id = 1`).
		WillReturnRows(sqlmock.NewRows([]string{
			...
			1, 100, 1, "mangadex", "", "", "secret-key", 1, "pk_test", "sk_test", "whsec_test", 1, 100, 60, 1, 5, 10, 60, 70, 85, 100, 90, 70, 85, 5, 3600, 3, 0, 0, "We are currently performing maintenance. Please check back later.", 48,
		))
