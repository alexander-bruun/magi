...
func ExtractAndStoreFirstImage(archivePath, slug, dataDir string, quality int) (string, error) {
	...
	return fmt.Sprintf("/api/posters/%s.%s", slug, fileExt), nil
}
...
// ExtractAndStoreImageWithCrop extracts an image (from a file path or archive) and stores it with optional cropping.
func ExtractAndStoreImageWithCrop(imagePath string, slug string, cropData map[string]any, quality int, useWebP bool) (string, error) {
...
func processCroppedImage(imagePath, slug, dataDir string, cropData map[string]any, quality int, useWebP bool) (string, error) {
...
func ExtractAndStoreImageWithCropByIndex(mangaPath, slug string, imageIndex int, cropData map[string]any, useWebP bool, quality int) (string, error) {
	dataDir := GetDataDirectory()
	postersDir := filepath.Join(dataDir, "posters")
	if err := os.MkdirAll(postersDir, 0755); err != nil {
		return "", fmt.Errorf("failed to create posters directory: %w", err)
